...
Use **`pack:<os>:server`** / **`pack:<os>:client`** (`linux`, `win`, **or** `mac`). These scripts use **`electron-builder.kord.cjs`**, optionally pass a semver as the **last argument** (e.g. `npm run pack:linux:server -- 3.0.0`; see **`scripts/pack-release.mjs`**).
