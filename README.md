...
Kord **2.3** ships with the behavior and packaging described below. **`package.json`** is the single source for the exact semver of a given commit.
...
Use **`pack:<os>:server`** / **`pack:<os>:client`** (`linux`, `win`, **or** `mac`). These scripts use **`electron-builder.kord.cjs`**, optionally pass a semver as the **last argument** (e.g. `npm run pack:linux:server -- 2.3.0`; see **`scripts/pack-release.mjs`**).
...
## Release 2.3 — notes for publishers
