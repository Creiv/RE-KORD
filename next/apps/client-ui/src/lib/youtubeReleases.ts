...
function releaseTypeToDiscoverKind(raw: string): YoutubeReleaseKind {
...
export function partitionYoutubeReleaseEntries<
  T extends YoutubeReleaseClassifyInput,
>(entries: T[]): { albums: T[]; songs: T[] } {
  const albums: T[] = [];
  const songs: T[] = [];
