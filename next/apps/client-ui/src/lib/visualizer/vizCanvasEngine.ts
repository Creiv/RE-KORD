...
export type VizCanvasDrawOptions = {
  width: number;
  height: number;
  mode: VizMode;
...
export class VizCanvasEngine {
  ...
  private softYs2: Float32Array | null = null;
  private softPulse = 0;
  private oscWaveScratch: {
    amp: Float32Array;
    ampSm: Float32Array;
    pref: Float32Array;
    xs: Float32Array;
  ...
  private oscSoftTape: Float32Array | null = null;
  private hmbScratch: {
    sig: Float32Array;
    blurB: Float32Array;
    blurM: Float32Array;
  ...
  resetForMode(mode: VizMode): void {
    ...
    if (mode !== "oscSoft") {
      this.oscWaveScratch = null;
      this.oscSoftTape = null;
    }
    if (mode !== "hmb") this.hmbScratch = null;
  }

  private refreshPalette(): void {
    const st = getComputedStyle(document.documentElement);
    const a =
      parseCssColor(st.getPropertyValue("--rk-accent")) ||
      parseCssColor(st.getPropertyValue("--accent"));
    const a2 =
      parseCssColor(st.getPropertyValue("--rk-accent-2")) ||
      parseCssColor(st.getPropertyValue("--accent2"));
    // Legacy uses --page-lg-* (deep page gradient), never card --surface.
    const p1 =
      parseCssColor(st.getPropertyValue("--rk-page-lg-1")) ||
      parseCssColor(st.getPropertyValue("--page-lg-1")) ||
      parseCssColor(st.getPropertyValue("--rk-bg-deep")) ||
      parseCssColor(st.getPropertyValue("--rk-bg")) ||
      parseCssColor(st.getPropertyValue("--bg"));
    const p2 =
      parseCssColor(st.getPropertyValue("--rk-page-lg-2")) ||
      parseCssColor(st.getPropertyValue("--page-lg-2")) ||
      parseCssColor(st.getPropertyValue("--rk-bg")) ||
      parseCssColor(st.getPropertyValue("--bg"));
    if (a) this.pal.accent = a;
    if (a2) this.pal.accent2 = a2;
    if (p1) this.pal.pageL1 = p1;
    else if (p2) this.pal.pageL1 = p2;
    if (p2) this.pal.pageL2 = p2;
  ...
  drawFrame(ctx: CanvasRenderingContext2D, opts: VizCanvasDrawOptions): void {
    ...
    const f = this.freq;
    const t = this.time;
    const tRef = { current: this.frame };

    if (mode === "karaoke") {
      ctx.clearRect(0, 0, w, h);
      const karaokeBg = ctx.createLinearGradient(0, 0, 0, h);
      karaokeBg.addColorStop(0, rgba(pal.pageL1, 0.98));
      karaokeBg.addColorStop(1, rgba(pal.pageL2, 0.99));
      ctx.fillStyle = karaokeBg;
    ...
    const gbar = (hh: number, y0: number) => {
      const g = ctx.createLinearGradient(0, y0, 0, hh);
    ...
    // scratch refs aliases
    const softXsRef = { current: this.softXs };
    const softYs0Ref = { current: this.softYs0 };
    const softYs1Ref = { current: this.softYs1 };
    const softYs2Ref = { current: this.softYs2 };
    const softPulseRef = { current: this.softPulse };
    const oscWaveScratchRef = { current: this.oscWaveScratch };
    const oscSoftTapeRef = { current: this.oscSoftTape };
    const hmbScratchRef = { current: this.hmbScratch };
    const peaksBars = { current: this.peaksBars };
    const peaksMir = { current: this.peaksMir };

    const syncScratch = () => {
      this.softXs = softXsRef.current;
      this.softYs0 = softYs0Ref.current;
      this.softYs1 = softYs1Ref.current;
      this.softYs2 = softYs2Ref.current;
      this.softPulse = softPulseRef.current;
      this.oscWaveScratch = oscWaveScratchRef.current;
      this.oscSoftTape = oscSoftTapeRef.current;
      this.hmbScratch = hmbScratchRef.current;
    };

    // Early returns in mode branches must still persist scratch (tape / silk pulse).
    try {
    ctx.clearRect(0, 0, w, h);
