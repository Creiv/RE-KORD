...
import {
  atomicWriteFileUtf8,
  ensureRekordSchemaFile,
  rekordAccountDir,
  rekordAccountLibrarySelectionPath,
  rekordAccountUserStatePath,
  rekordBaseDir,
  rekordConfigPath,
  rekordGlobalInfoDir,
  ensureKordDataDirOnDisk,
  readJsonFile,
...
export async function migrateMoodsFromTrackInfoToUserState(libraryRoot, userStateJsonPath) {
  ...
  async function scanAlbumDir(albumDir, artistName, albumFolder) {
    const tp = existingAlbumTrackInfoPath(albumDir)
    if (!tp) return
    let j = {}
    try {
  ...
  try {
    ...
    for (const e of top) {
      ...
      if (e.name === "node_modules" || e.name === ".kord" || e.name === ".kord") continue
 ...
 */
export async function runRekordLayoutMigration({
  libraryRoot,
  accounts,
  ...
  await ensureKordDataDirOnDisk(root)
  await ensureRekordSchemaFile(root)
  const base = rekordBaseDir(root)
  const gInfo = rekordGlobalInfoDir(root)
  ...
  if (fs.existsSync(legacyAccountsTree)) {
    ...
    for (const s of subs) {
      ...
      const dest = rekordAccountDir(root, s.name)
      if (!dest) continue
  ...
  const newKordConfig = rekordConfigPath(root)
  ...
  for (const acc of accounts) {
    ...
    if (!id) continue
    const accDir = rekordAccountDir(root, id)
    if (!accDir) continue
    await fsp.mkdir(accDir, { recursive: true })

    const destState = rekordAccountUserStatePath(root, id)
    ...
    if (destState && !fs.existsSync(destState)) {
      ...
      const legMistaken = path.join(root, ".rekord", "user-state.v1.json")
      const legWpp = path.join(root, ".wpp", "user-state.v1.json")
      const pick = fs.existsSync(legKord)
        ? legKord
        : fs.existsSync(legMistaken)
          ? legMistaken
          : fs.existsSync(legWpp)
            ? legWpp
            : null
      if (pick && id === "default") {
        try {
    ...
    const selPath = rekordAccountLibrarySelectionPath(root, id)
  ...
  for (const acc of accounts) {
    const id = String(acc?.id || "").trim()
    if (!id) continue
    const up = rekordAccountUserStatePath(root, id)
