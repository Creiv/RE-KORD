...
export function RekordSplashLoader() {
  const { t } = useI18n();
  ...
  return (
    <div
      className="rekord-splash"
      role="status"
      aria-label={String(t("loading.splashAria"))}
    >
      <div
        className="rekord-splash__carousel"
        aria-live="polite"
        aria-atomic="true"
      >
        <p className="rekord-splash__tip" key={tipIndex}>
          {String(t(tipKeys[tipIndex]))}
        </p>
      </div>
      <h1 className="rekord-splash__title">{t("loading.splashTitle")}</h1>
      <p className="rekord-splash__lead">{t("loading.splashLead")}</p>
      <div className="rekord-splash__spinner" aria-hidden>
